...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        deta_q_list = [deta_q] * batch.max_seq_length#改写,初始化deta_q_list
        for t in range(batch.max_seq_length):
            ...
            # Calculate deta_q
            deta_q = q_com - q_ncom[args.n_agents - 1]
            
            # Expand deta_q for each time step  
            deta_q_list[t]=deta_q
            # Gated end
        ...
        #改写
        # Sort deta_q_list
        deta_q_list.sort()#L_deta_Q[],升序
        
        # Compare deta_q_list with current deta_q_list for each agent
        Y_o = []
        for i in range(args.n_agents):
            threshold = args.prob * len(deta_q_list)#T=threshold
            if deta_q_list[i] > threshold:
                Y_o.append(1)
            else:
                Y_o.append(0)
